...
ASGEServer::ASGEServer(const ASGE::GameSettings& settings) : OGLGame(settings),net_server(std::make_unique<GCNetServer>(this)) {
	//========= Input ==========//
	inputs->use_threads = false;
	inputs->addCallbackFnc(ASGE::EventType::E_KEY, &ASGEServer::keyHandler, this);
	inputs->addCallbackFnc(ASGE::EventType::E_MOUSE_MOVE, &ASGEServer::mouseMoveHandler, this);
	inputs->addCallbackFnc(ASGE::EventType::E_MOUSE_CLICK, &ASGEServer::mouseClickHandler, this);
...
//					 update
//================================================//
void ASGEServer::update(const ASGE::GameTime& us)
{		
	if (gameEnd ) {
		return;
	}
	...
	if (isGameStarted() && clientReady) {
		if (!currentTurn) return;

		if (score >= 55) {
			gameEnd = true;
			win = true;
			net_server->send(Packet{ -1, -1, -1,false,true });
		}
	}
	...
	if (mouse_buttons[ASGE::MOUSE::MOUSE_BTN1]) {
		...
		if (xCursor >= 15 && xCursor < 23 && yCursor >= 0 && yCursor <= 8) {
			net_server->send(Packet{ (xCursor - 15), yCursor, -1, true, false });
		}
...
//					 render
//================================================//
void ASGEServer::render(const ASGE::GameTime& /*us*/) {
...
void ASGEServer::keyHandler(ASGE::SharedEventData data)
{
...
void ASGEServer::mouseClickHandler(ASGE::SharedEventData data) {
...
//				mouseMoveHandler 
//=================================================//
void ASGEServer::mouseMoveHandler(ASGE::SharedEventData data) {
...
//=================================================//
// add ship to the map
void ASGEServer::addShip(const Ship& ship) {
...
std::tuple<int, int> ASGEServer::calculateTileIndex() {
...
bool ASGEServer::isGameStarted() {
	if (!startTheGame) {
		...
		startTheGame = true;
		net_server->send(Packet{ -1, -1, -1, true, false });
	}
	return startTheGame;
}
...
void ASGEServer::handlePacket(Packet packet) {
	...
	if (!clientReady) {
		...
		int hit = 0;
		if (packet.hit == -1) {
			/* the server recieve a hit, so we check if any ship get hit but the client, 
			and send which ship get hit to the client*/			
			currentTurn = true;
			...
			switch (tiles[packet.y][packet.x]) {
				...
				case Tile::TileType::SEA:					
					...
					break;
				default:					
					return; //dont send any packet
			}
			net_server->send(Packet{ packet.x, packet.y, hit, true, false });
		} 
		else {		
			/*after we play our turn, we check here if any opponent ship get hit */	
